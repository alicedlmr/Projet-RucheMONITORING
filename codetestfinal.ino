...
bool extractDownlink(const String &acc, int &port, String &hex) {
  ...
  if (pPort >= 0) {
    ...
    if (pSep >= 0) {
      ...
      String num;
      while (i < (int)up.length() && isDigit(up[i])) { num += up[i]; i++; }
...
void applyDownlink(int port, const String &hex) {
  ...
  if (cmd == 0x01 && hex.length() >= 4) {
    uint8_t mm;
    if (hexByte(hex.c_str() + 2, mm) && mm >= 1) {
      overrideMin = mm;
      periodOverride = true;
  ...
  if (cmd == 0x02 && hex.length() >= 6) {
    uint8_t hi, lo;
    if (hexByte(hex.c_str() + 2, hi) && hexByte(hex.c_str() + 4, lo)) {
  ...
  if (cmd == 0x05 && hex.length() >= 4) {
    uint8_t mm;
    if (hexByte(hex.c_str() + 2, mm)) {
      if (mm == 10 || mm == 15) {
        highPeriodMin = mm;
...
TxResult envoyerPayloadLoRa(const char *payloadHex, uint32_t timeoutMs) {
  ...
  while (millis() - t0 < timeoutMs) {
    while (Serial2.available()) {
      char c = (char)Serial2.read();
      acc += c;
      if (acc.length() > 700) acc.remove(0, 350);

      String up = acc; up.toUpperCase();
      if (up.indexOf("PLEASE JOIN NETWORK FIRST") >= 0) r.joinNeeded = true;
      if (up.indexOf("+MSGHEX: DONE") >= 0 || up.indexOf("DONE") >= 0) r.done = true;
